...
**Related to:** [Documentation Overview](00-overview.md) — Area 2: Runbook Standards
...
Steps that are too vague for safe Claude Code execution — "ensure the database is healthy," "check that the deployment succeeded" — are also too vague for a new engineer following the runbook during an incident. The runbook writing discipline that produces Claude-executable procedures is identical to the discipline that produces reliable human-following procedures. The test is not "would an expert understand this step?" but "would a non-expert in a high-stress situation perform this step safely and know when it had succeeded?"[^4]
...
- Limit Claude Code runbook execution to runbooks that have been marked as execution-verified: runbooks that have been successfully executed by Claude Code in a staging environment at least once, with an engineer monitoring. Unverified runbooks may have steps that are ambiguous in ways that only surface during machine execution — discover these in staging, not production.[^6]
...
The runbook maintenance failure mode is discovered at the worst possible time: during a production incident, when an engineer follows a runbook that describes a system that no longer exists. The deployment procedure in the runbook references a CI/CD pipeline step that was refactored two sprints ago; the database migration procedure references a schema that was changed in an AI-generated PR last week; the authentication recovery procedure references an endpoint that was renamed. Each of these is a runbook gap that accumulated silently during routine development and becomes visible only under operational pressure.[^4]
...
[^4]: GitClear — "Coding on Copilot: 2025 Data Suggests Downward Pressure on Code Quality," GitClear Research, 2025. https://www.gitclear.com/coding_on_copilot_data_suggests_coding_quality_is_decreasing
    Runbook currency failure as an AI-assisted development risk: how fast system change velocity outpaces runbook maintenance cadences designed for human-paced development.
...
[^6]: Gartner — "Predicts 2026: Software Engineering and DevSecOps," Gartner Research, January 2026. https://www.gartner.com/en/documents/predicts-2026-software-engineering-devsecops
    Runbook verification as an operational hygiene practice; staging verification before production execution; the monthly verification cadence and its relationship to incident readiness.
