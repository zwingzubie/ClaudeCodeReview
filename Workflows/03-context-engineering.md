...
- When Anthropic releases a new Claude model version and the team adopts it for standard sessions, treat the transition as a mandatory context recalibration event before team-wide rollout. New models have different default behaviors, different failure modes, and different baseline conventions — a `CLAUDE.md` written to correct one model's specific errors may contain instructions that are unnecessary, irrelevant, or counterproductive for the next model. Schedule a CLAUDE.md calibration session with the architect before the new model is used in standard sessions: run the CLAUDE.md test checklist (see Tooling & Configuration — CLAUDE.md Configuration, Section 5) against the new model, prune instructions the new model follows without prompting, and add instructions for new failure modes the transition reveals.[^2]
- Version the `CLAUDE.md` and imported instruction files in git with meaningful commit messages that reference the model version they were validated against: `"Calibrated for claude-sonnet-4-6: pruned 3 instructions, added 2 auth corrections."` This creates a change history that makes it possible to identify when a specific instruction was added, what model behavior it was correcting, and whether it remains relevant to the current model. A `CLAUDE.md` with no version history is harder to prune responsibly because the reason each instruction was added is lost.[^3]
