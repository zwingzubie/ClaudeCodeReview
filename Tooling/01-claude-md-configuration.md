...
The validation method is direct: ask Claude to behave in ways that the CLAUDE.md would prevent, and verify that it declines or asks for clarification. Ask it to implement a pattern that is explicitly prohibited; ask it to omit a required test type; ask it to use an off-limits library. A correctly functioning CLAUDE.md should produce either refusal or a note that the request conflicts with team instructions. If it produces neither, the relevant instruction needs revision.[^3]

**Recommended Practice:**
- Create a CLAUDE.md test checklist: a short list of behaviors that should be prevented or required by the file's current content. Run this checklist in a fresh session after any significant update to verify the changes had the intended effect.[^4]
- If an instruction fails validation (Claude does not follow it), diagnose before rewriting. Common causes: the instruction is buried too far into a long file, it conflicts with another instruction, or it is phrased as a preference rather than a prohibition. Address the root cause rather than restating the instruction more forcefully.[^2]
