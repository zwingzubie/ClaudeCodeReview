...
- Review the token economics of the team's most common session types: estimate the typical input context, number of turns, and output volume for each session pattern. This exercise identifies which session patterns are disproportionately expensive relative to their output value.[^4]
...
- Use targeted file injection rather than whole-directory injection. When Claude Code needs to understand a module, inject the specific files relevant to the task rather than the entire module directory. The quality difference on a focused task is small; the token difference can be 5–10× for a large module.[^4]
...
[^4]: The Pragmatic Engineer — "AI Tooling for Software Engineers in 2026," March 2026. https://newsletter.pragmaticengineer.com/p/ai-tooling-2026
    Context as the primary cost driver in Claude Code sessions; the inversion of prompt-length intuition; cost-per-session analysis across common engineering task types; targeted file injection vs. directory injection patterns.
